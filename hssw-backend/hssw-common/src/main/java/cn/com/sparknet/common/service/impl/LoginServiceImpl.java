...
public class LoginServiceImpl implements LoginService {
    ...
    public LoginServiceImpl(){
        String loginErrorCount = Config.getInstance().getProperty("common.login.error.count");
                ...
                if(errorCountValid==true){
                    if(map.get("STATE").toString().equals("L")){
                        ...
                        if(dataBaseTime.size() > 0){
                            ...
                            //读取配置文件
                            String unlockWait = Config.getInstance().getProperty("common.login.error.unlockWaitTime");
                            //用户锁定时间
                            if(null != map.get("LOCK_DATE")){
                                ...
                                if(unlock < result){
                                    ...
                                    editJson.setBean("该账户已被锁定，请等待"+unlockWait+"分钟之后重新登录或者联系管理员进行解锁！");
    ...
    private void validCorrectness(EditJson editJson,HttpServletRequest request,HttpSession session,
        ...
        try{
            if(MD5Util.encrypt(password).equals(map.get("USER_PASSWORD"))){
                //更新用户登录时间、登录错误次数
                ...
                // 检测session是否有效(测试)
                System.out.println("当前登录的系统用户名: " + session.getAttribute("USER_NAME"));
                //上次登录时间
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                ...
                if(errorCountValid==true){
                    //更新用户登录时间、登录错误次数
                    if(currentErrorCount==(errorCount-1)){
